...
namespace Qualia.Controls.Viewer;

public sealed unsafe partial class RandomizerViewer : WindowResizeControl
{
    ...
    private void Render()
    {
        ...
        var left = CtlPresenter.CtlPresenter.Width / 2 - (_model.Layers.Count - 1) * (layersDistance - 100 / 2) / 2;
        var top = CtlPresenter.CtlPresenter.Height / 2 - _model.Layers.First.Neurons.Count / 2;

        byte alpha = 100;
        var heightOf1 = (CtlPresenter.CtlPresenter.Height - 250) / 2 / GetModelWeightsMaxValue();

        using Pen zeroColor = new(Color.FromArgb(alpha, Color.Gray)); 

        for (var layer = 0; layer < _model.Layers.Count - 1; ++layer)
        {
            var neuronsCount = _model.Layers[layer].Neurons.Count;
            for (var neuron = 0; neuron < neuronsCount; ++neuron)
            {
                ...
                for (var weight = 0; weight < _model.Layers[layer].Neurons[neuron].Weights.Count; ++weight)
                {
